...
def generator_loss(gen_output, disc_output):
    return huber_loss(tf.ones_like(disc_output), disc_output)

##### A different method for calculating loss for the discriminator might be used
#def discriminator_loss(real_output, fake_output):
#    real_loss = huber_loss(tf.ones_like(real_output), real_output - 1)
#    fake_loss = huber_loss(tf.zeros_like(fake_output), fake_output)
#    return real_loss + fake_loss
#####

##### Initial loss function, might be removed later
...
noise_for_generation = tf.random.normal([num_images_to_generate, 100])
...
for i in range(num_images_to_generate):
    ...
    plt.imshow(generated_images[i, :, :, 0] * 127.5 + 127.5, cmap='gray')  # Denormalize
    plt.axis('off')
plt.show()
